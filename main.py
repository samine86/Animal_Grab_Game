...
class SettingWindow(Screen):
    def __init__(self):
        ...
        self.add_widget(Button(text='Start Game', pos_hint={'x': .02, 'y': .02}, size_hint=(.95, .1), on_press=self.start_game))
...
class GameWindow(Screen):
    ...
    def game_draw(self, dt):
        ...
        self.canvas.add(Rectangle(pos=(Window.width // 2, 35), size=self.TimeLabel.texture.size, texture=self.TimeLabel.texture))


        # Remove Animals Going outside of the screen Loop
        ...
        # Add New Animals To Animal Container List Loop
...
class ResultWindow(Screen):
    def __init__(self):
        ...
        self.add_widget(Label(text='Your Score : {}'.format(Public.score), pos_hint={'x':.35, 'y': .5}, size_hint=(.3, .1), font_size= 40))
        self.add_widget(Button(text='Restart Game', pos_hint={'x':.02, 'y': .02}, size_hint=(.5, .1), on_press=self.game_restart))
        self.add_widget(Button(text='Exit Game', pos_hint={'x': .53, 'y': .02}, size_hint=(.46, .1), on_press=self.game_exit))
